...
const EditSkillsDialog = ({
  ...
  const SkillsSchema = Yup.object().shape({
    ...
    newSkill: Yup.string().when('skills', {
      is: (skills: string[]) => skills.length === 0,
      then: (schema) => schema.required('Please add at least one skill'),
              ...
              <Grid item xs={12}>
                <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                  <RHFTextField
                    ...
                    helperText={errors.newSkill?.message}
                  />
                  <Button
                    variant="outlined"
